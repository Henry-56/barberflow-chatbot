...
export async function procesarMensaje(phone, texto, nombreContacto) {
  ...
  if (s.estado === E.ESPERANDO_DECISION) {
    if (preguntaPrecio(texto)) {
      await sendMessage(phone,
        `BarberFlow tiene una prueba gratuita de *21 días* — sin tarjeta, sin compromiso. 🎁\n\n` +
        `Después de la prueba son *S/29 al mes*. Pero primero úsalo gratis y decides. 💡\n\n` +
        `Mejor te lo muestro en vivo → así ves exactamente cuánto genera en tu caso.`
      );
      await enviarHorarios(phone);
      return;
    }
